...
test('Verify checkout alert with one product', async ({ shopPage  }) => {
  const checkoutAlert = handleCheckoutAlert(shopPage.page, testData.products[0].price);
  ...
  await test.step('Then the product should be visible in the cart', async () => {
    const productRow = shopPage.getProductRowInCart(testData.products[0].name);
    await expect(productRow.rootEl).toBeVisible();
  });

  await test.step('When checkout button is clicked', async () => {
    await shopPage.cart.checkoutButton.click();
  });

  await test.step('Then checkout alert is shown with correct message', async () => {
    await checkoutAlert;
  });
});
